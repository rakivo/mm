...
impl Inst {
    pub fn as_bytes(&self) -> Vec::<u8> {
        match self.typ {
            ...
            InstType::U2I     => vec![35],
            InstType::U2F     => vec![36],
            InstType::NATIVE  => extend_to_bytes_string(37, self.val.as_string()),
            InstType::READ8   => vec![38],
            InstType::WRITE8  => vec![39],
            InstType::READ16  => vec![40],
            InstType::WRITE16 => vec![41],
            InstType::READ32  => vec![42],
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result::<(Inst, usize), Trap> {
        match bytes.first() {
            ...
            Some(38) => Ok((Inst::READ8, 1)),
            Some(39) => Ok((Inst::WRITE8, 1)),
            Some(40) => Ok((Inst::READ16, 1)),
            Some(41) => Ok((Inst::WRITE16, 1)),
            Some(42) => Ok((Inst::READ32, 1)),
